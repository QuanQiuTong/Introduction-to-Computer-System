...
bool do_free(int argc, char *argv[]) {
    ...
    if (bcnt > 0) {
        report(1, "ERROR: Freed _free_ queue, but %lu blocks are still allocated",
               bcnt);
        ok = false;
    }
    return ok && !error_check();
...
bool do_remove_head(int argc, char *argv[]) {
    ...
    if (rval) {
        ...
        if (!check && fail_count < fail_limit) {
            ...
            report(1, "ERROR:  Removal _not_quiet from queue failed (%d failures total)",
                   fail_count);
            ok = false;
        }
    }
...
bool do_remove_head_quiet(int argc, char *argv[]) {
    ...
    if (rval) {
        ...
        if (fail_count < fail_limit)
            ...
            report(1, "ERROR: Removal _quiet failed (%d failures total)", fail_count);
            ok = false;
        }
    }
    show_queue(3);
...
static bool queue_quit(int argc UNUSED, char *argv[] UNUSED) {
    ...
    size_t bcnt = allocation_check();
    if (bcnt > 0) {
        report(1, "ERROR: Freed _quit_ queue, but %lu blocks are still allocated",
               bcnt);
        return false;
    }
    return true;
